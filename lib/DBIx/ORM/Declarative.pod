...
The aliases created using the C<table_aliases> key can be used any place
the aliased table would be used.  This is useful, for example, if you have
a table named C<address>, and aliases of C<billing_addr> and C<shipping_addr>.
Not only does this tend to make code easier to read, since you're able to
use a method that reflects the actual use of the object it operates on,
it's the easiest way to create a join that uses a table more than once.
